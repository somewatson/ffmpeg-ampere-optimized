...
RUN cd /ffmpeg_sources && \
    ...
    cd x264 && \
    ./configure --prefix="/ffmpeg_build" --bindir="/bin" --enable-static --enable-pic --extra-cflags="-mcpu=neoverse-n1 -O3" && \
    make -j $(nproc) && \
    make install

# Build libx265
RUN export CFLAGS="-mcpu=neoverse-n1 -O3" && \
    export CXXFLAGS="-mcpu=neoverse-n1 -O3" && \
    cd /ffmpeg_sources && \
    git clone --depth 1 https://bitbucket.org/multicoreware/x265_git.git x265 && \
    cd x265/build/linux && \
    cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="/ffmpeg_build" -DENABLE_SHARED=off -DENABLE_SVE=OFF -DCMAKE_C_FLAGS="-mcpu=neoverse-n1 -O3" -DCMAKE_CXX_FLAGS="-mcpu=neoverse-n1 -O3" ../../source && \
    cmake --build . -j $(nproc) && \
    make install && \
...
RUN cd /ffmpeg_sources/ffmpeg && \
    PKG_CONFIG_PATH="/ffmpeg_build/lib/pkgconfig" ./configure \
        ...
        --extra-cflags="-I/ffmpeg_build/include -mcpu=neoverse-n1 -O3" \
        --extra-cxxflags="-mcpu=neoverse-n1 -O3" \
