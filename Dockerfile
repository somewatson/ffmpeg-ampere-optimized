...
RUN cd /ffmpeg_sources && \
    ...
    cd x264 && \
    ./configure --prefix="/ffmpeg_build" --bindir="/bin" --enable-static --enable-pic --extra-cflags="-mcpu=neoverse-n1 -flto=auto" && \
    make -j $(nproc) && \
    make install
...
RUN export CFLAGS="-mcpu=neoverse-n1 -flto=auto" && \
    ...
    cd x265/build/linux && \
    cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="/ffmpeg_build" -DENABLE_SHARED=off -DENABLE_SVE=OFF -DCMAKE_C_FLAGS="-mcpu=neoverse-n1 -flto=auto" -DCMAKE_CXX_FLAGS="-mcpu=neoverse-n1 -flto=auto" ../../source && \
    cmake --build . -j $(nproc) && \
    make install && \
...
RUN cd /ffmpeg_sources/ffmpeg && \
    PKG_CONFIG_PATH="/ffmpeg_build/lib/pkgconfig" ./configure \
        ...
        --extra-cflags="-I/ffmpeg_build/include -mcpu=neoverse-n1 -flto=auto" \
        --extra-cxxflags="-mcpu=neoverse-n1 -flto=auto" \
        --extra-ldflags="-L/ffmpeg_build/lib -flto=auto" \
